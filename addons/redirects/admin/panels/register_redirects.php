...
<p><?php _e('The top rules always have higher priority. So any new rule you add will be higher in priority and checked first in the redirection rules during login or registering (depending on where you set the redirection).','userpro'); ?></p>
<p><?php _e('You can setup a rule for a specific role, or a specific user, or perhaps redirect all users who choose United States as country to be redirected to a custom page. This addon also supports automatic mapping. Please check the help tab for examples and help.','userpro'); ?></p>
<table class="form-table">
	...
	<tr valign="top">
		<td>
			...
			<p>
			<span class="description"><?php _e('If you want redirection based on specific <strong>custom field value</strong> enter the value here: (e.g. country or city) This is helpful if you wish to redirect users who have a specific city in profile, be redirected to a custom page after their login. If you wish to <strong>auto-map</strong> or redirect users automatically based on the custom field value you could use wildcards like this: <strong>{your_custom_field}</strong> in the redirection URL. The wildcard will also work on {username}','userpro'); ?></span>
			...
			<span class="description"><?php _e('You can use wildcards for automated mapping. For example, if you enter {username} that will be replaced automatically by the username in redirection url. If you want to map a custom field <strong>country</strong> then use the wildcart {country} to map the user to his country automatically. If you use wildcards, the field value is not required.','userpro'); ?></span>
...
<h3><?php _e('Existing Register Redirects','userpro'); ?></h3>

<div id="register_redirects" class="upadmin-redirects">
	<table class="wp-list-table widefat fixed">
		<?php echo userpro_rd_list_redirects('register'); ?>
	</table>
</div>
