<?php
	...
	function userpro_profile_icons_noargs( $user_id, $wrapper=null ) {
		...
		foreach( userpro_fields_group_by_template( 'social', 'default' ) as $key => $array ) {
			...
			$res .= '<a href="'.userpro_link_filter( userpro_profile_data($key, $user_id), $key ).'" class="userpro-profile-icon" target="_blank" ><i class="userpro-icon-'.$icon.'"></i></a>';
			}
		}
		if ($res){
			if ($wrapper){
	...
	function userpro_profile_icons( $args, $user_id, $wrapper=null ) {
		...
		foreach( userpro_fields_group_by_template( 'social', $args["social_group"] ) as $key => $array ) {
			...
			$res .= '<a href="'.userpro_link_filter( userpro_profile_data($key, $user_id), $key ).'" class="userpro-profile-icon" target="'.$args['social_target'].'" ><i class="userpro-icon-'.$icon.'"></i></a>';
			}
		}
		if ($res){
			if ($wrapper){
